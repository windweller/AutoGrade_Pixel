...
def train_normal_6M():
    ...
    name = "standard"
    wandb.init(sync_tensorboard=True, project="autograde-bounce",
               name="paper_train_graph_hardcourt_{}".format(name))
    ...
    with tf.Session(config=config):
        checkpoint_callback = CheckpointCallback(save_freq=250000,
                                                 save_path="./saved_models/paper_train_graph_hardcourt_{}/".format(name),
                                                 name_prefix="ppo2_cnn_lstm_default")
        ...
        model = PPO2('CnnLstmPolicy', env, n_steps=256, learning_rate=5e-4, gamma=0.99,
                     verbose=1, nminibatches=4,
                     tensorboard_log="./tensorboard_paper_train_graph_hardcourt_standard_6M_log/")
        steps = 6000000
        save_name = 'standard'
        ...
        model.save("./saved_models/paper_train_graph_hardcourt_{}".format(save_name))
