...
class AnomalyDataset(tDataset):
    def __init__(self, pos_dir=None,
        ...
        self.pos_fnames = [f for f in glob.glob(pos_dir + "/*.mp4", recursive=False)] if pos_dir is not None else []
        self.neg_fnames = [f for f in glob.glob(neg_dir + "/*.mp4", recursive=False)] if neg_dir is not None else []
    ...
    def __getitem__(self, index):
        ...
        target = 1 if 'correct_sample' in self.fnames[index] else 0
        assert '.mp4' in self.fnames[index]
...
if __name__ == '__main__':
    ...
    # Note: video length is frameskip * max_frames (for 2000 frames, we can do frameskip=5, max_frame=200)
    ...
    if args.output_dir is None:
        output = os.path.join("output", "video",
                              "{}_{}_{}_{}".format(args.modelname, max_frames, args.frameskip, "pretrained" if args.pretrained else "random"))
    else:
    ...
    d_test = AnomalyDataset(pos_dir=None,
                            neg_dir="/data/anie/AutoGrade/rec_vidoes_toy_programs/external_neg_test",
    ...
    # TODO: there are numerous mistakes here. For test folder, we should not shuffle!
